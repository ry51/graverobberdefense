function animate() {
    ...
    if (wavetimer <= 0) {
		if (wave % 50 != 0 && doom == true) {
			if (wave > 150) {
				if (asc == true) {
					spawnEnemy(3000 + 1000*spireBoost + Math.random()*(7000 - 1000*spireBoost), 3000 + 1000*spireBoost + Math.random()*(7000 - 1000*spireBoost), 40, (200 + time*0.8)*1.05**wave*45.87*(1.04/1.05)**(wave-ascWave)*bm*25, (200 + time*0.8)*1.05**wave*45.87*(1.04/1.05)**(wave-ascWave)*25, 8, "#550000", 1, 20, 20, (6 + time*0.001)*1.01**wave*3.289*(1.008/1.01)**(wave-ascWave)*2, './assets/enemies/doomenemy.png');
				} else {
					spawnEnemy(3000 + 1000*spireBoost + Math.random()*(7000 - 1000*spireBoost), 3000 + 1000*spireBoost + Math.random()*(7000 - 1000*spireBoost), 40, (200 + time*0.8)*1.05**wave*45.87*bm*25, (200 + time*0.8)*1.05**wave*45.87*25, 8, "#550000", 1, 20, 20, (6 + time*0.001)*1.01**wave*3.289, './assets/enemies/doomenemy.png');
				}
			} else {
				if (asc == true) {
					spawnEnemy(3000 + 1000*spireBoost + Math.random()*(7000 - 1000*spireBoost), 3000 + 1000*spireBoost + Math.random()*(7000 - 1000*spireBoost), 40, (200 + time*0.8)*1.05**wave*45.87*(1.04/1.05)**(wave-ascWave)*bm, (200 + time*0.8)*1.05**wave*45.87*(1.04/1.05)**(wave-ascWave), 8, "#550000", 1, 20, 20, (6 + time*0.001)*1.01**wave*3.289*(1.008/1.01)**(wave-ascWave)*2, './assets/enemies/doomenemy.png');
				} else {
					spawnEnemy(3000 + 1000*spireBoost + Math.random()*(7000 - 1000*spireBoost), 3000 + 1000*spireBoost + Math.random()*(7000 - 1000*spireBoost), 40, (200 + time*0.8)*1.05**wave*45.87*bm, (200 + time*0.8)*1.05**wave*45.87, 8, "#550000", 1, 20, 20, (6 + time*0.001)*1.01**wave*3.289, './assets/enemies/doomenemy.png');
				}
			}
			
		}
		for (let i = 0; i < Math.floor(wave/5) + 1; i++) {
	...
	ctx.fillText("Enemies in " + Math.floor(wavetimer / 60), canvas.width / 2, 95);
    ctx.textAlign = "left";
	...
	if (intropage == 0) {
		...
		ctx.font = "90px Courier New";
		ctx.textAlign = "center";
		ctx.fillText("Graverobber Defense", canvas.width / 2, canvas.height / 2 - 240);
		ctx.drawImage(getImage("./assets/misc/pyramid.png"), canvas.width / 2 - 100, canvas.height / 2 - 200, 200, 200);
		ctx.font = "30px Courier New";
	...
	if (double == true && pause % 2 == 0 && time % 3 == 0) {
